...
(function () {
  ...
  var minuteVolumeRatio = 1.5, minuteVolumeMode = 'stock', minuteVolumeLocalEvents = [];
  ...
  var HISTORY_WATCHLIST = null, HISTORY_SOURCE_POOLS = {}, historyAssetsLoading = false;
  ...
  var MEMBER_WORKBENCH_VERSION = '1.0.25';
  ...
  function loadMemberLocalRealtime() {
    ...
    return new Promise(function (resolve) {
      ...
      script.src = 'http://127.0.0.1:8790/api/compat?callback=' + callback + '&action=realtime&member_id=' + encodeURIComponent(memberId) + '&_=' + Date.now();
  ...
  function render() {
    ...
    if (currentView === 'signal' || currentView === 'leading') {
      var signalView = signalRealtimeMode && signalRealtimePayload && signalRealtimePayload.available && signalRealtimePayload.data_date === localToday()
        ? Object.assign({}, view, { limitup: signalRealtimePayload.limitup || [], events: signalRealtimePayload.events || [],
  ...
  function vMinuteVolume(payload) {
    ...
    var controls = '<header class="minute-toolbar"><div class="minute-toolbar-title"><strong>分钟爆量雷达</strong><span class="live-dot"></span><small>' +
      ...
      '<div class="minute-filter-group minute-ratio-filter"><button data-minute-ratio="1">超昨峰</button><button data-minute-ratio="1.5" class="' + (minuteVolumeRatio === 1.5 ? 'active' : '') + '">≥1.5×</button><button data-minute-ratio="2" class="' + (minuteVolumeRatio === 2 ? 'active' : '') + '">≥2.0×</button></div>' +
      '<div class="minute-quality">分钟覆盖 <b>' + esc(quality.coverage == null ? '-' : (Number(quality.coverage) * 100).toFixed(1) + '%') + '</b> · 有效 ' + esc(quality.valid_stocks || '-') + '只</div></header>';
    ...
    var tableRows = minuteVolumeMode === 'sector' ? (payload.sectors || []).map(function (row, index) {
      ...
      return '<tr class="minute-stock-row' + (selected ? ' selected' : '') + '" data-minute-sid="' + esc(row.stock_id) + '"><td>' + (index + 1) + '</td><td class="l"><b>' + stk(row.stock_id, row.name) + '</b><small>' + esc((row.sectors || [])[0] || '未分类') + '</small></td><td class="' + cls(row.change_pct || 0) + '">' + esc(row.change_pct == null ? '-' : fmtPct(row.change_pct)) + '</td><td><b class="minute-ratio-badge">' + esc(Number(row.volume_ratio).toFixed(2)) + '×</b></td><td><span class="minute-type-badge">' + esc(row.price_volume_type) + '</span></td><td>' + esc(row.minute) + '</td><td>' + esc(row.sector_sync_count || 0) + '只</td><td>' + minuteVolumeWatchButton(row) + '</td></tr>';
    }).join('');
  ...
  function refreshMinuteVolume() {
    ...
    fetchJSON('api/minute-volume?ratio=' + minuteVolumeRatio + selected + dateArg, 'no-cache').then(function (result) { minuteVolumePayload = result.data || result; if (minuteVolumePayload.detail) minuteVolumeSelectedId = minuteVolumePayload.detail.stock_id; render(); if (currentView === 'minute-volume') minuteVolumeTimer = window.setTimeout(refreshMinuteVolume, 60000); }).catch(function (error) { minuteVolumePayload = { available: false, reason: '分钟爆量接口读取失败：' + error.message, quality: { status: 'fail' } }; render(); if (currentView === 'minute-volume') minuteVolumeTimer = window.setTimeout(refreshMinuteVolume, 60000); });
  }
  ...
  function signalDateStatus(view) {
    if (view.signal_ts) return localToday() + ' · 实时 ' + String(view.signal_ts).slice(11, 19);
  ...
  document.addEventListener('click', function (ev) {
    ...
    var minuteRatio = ev.target.closest ? ev.target.closest('[data-minute-ratio]') : null;
    if (minuteRatio && currentView === 'minute-volume') { minuteVolumeRatio = Number(minuteRatio.dataset.minuteRatio) || 1; minuteVolumeSelectedId = null; startMinuteVolume(); return; }
