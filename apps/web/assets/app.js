...
(function () {
  ...
  var MEMBER_WORKBENCH_VERSION = '1.0.42';
  ...
  var intradayCoreRequest = null, intradayCoreRequestedAt = 0;
  function fetchIntradayCore() {
    var now = Date.now();
    if (intradayCoreRequest && now - intradayCoreRequestedAt < 1000) return intradayCoreRequest;
    intradayCoreRequestedAt = now;
    intradayCoreRequest = fetchJSON('api/intraday/latest?scope=core', 'no-store');
    intradayCoreRequest.catch(function () { intradayCoreRequest = null; });
    return intradayCoreRequest;
  }
  ...
  function mergeMemberLocalRealtime(payload, local) {
    ...
    if (local.history_pools) {
      payload.history_pools = local.history_pools;
      payload.history_pool_summary = local.history_pool_summary || {};
    }
  ...
  function postJSON(url, payload) {
    function request(retried) {
      return fetch(url, { method: 'POST', cache: 'no-store', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload) }).then(function (r) { return r.json().then(function (d) {
          if (!r.ok && r.status === 403 && d.error === 'local token required' && !retried && memberIsLocalWorkbench()) {
            return fetch('/index.html', { cache: 'no-store', credentials: 'same-origin' }).then(function (page) {
              if (!page.ok) throw new Error('本地安全会话恢复失败，请刷新页面');
              return request(true);
            });
          }
  ...
  function licenseJSON(path, payload) {
    ...
    return postJSON(MEMBER_LICENSE_API + path, payload).then(function (doc) {
      if (!doc.success) throw new Error(doc.message || doc.error || '授权校验失败'); return doc;
    })
      .catch(function (err) {
  ...
  function persistMemberLicense(doc, code) {
    var user = doc.user || {};
    memberLicenseState = { valid: true, code: memberIsLocalWorkbench() ? '' : code, cloud_user_id: user.id, user: user,
      device_fingerprint: memberIsLocalWorkbench() ? '' : memberDeviceFingerprint(), last_check: new Date().toISOString() };
    ...
    if (signalRealtimeMode) startSignalRealtime();
    if (stratRealtime) { window.clearTimeout(stratRealtimeTimer); refreshStratRealtime(); }
  ...
  function validateMemberLicense() {
    ...
    if ((!state || !state.code) && !memberIsLocalWorkbench()) { renderMemberLicense(null, '请先输入激活码'); return Promise.resolve(null); }
    if ($('memberLicenseMessage')) $('memberLicenseMessage').textContent = '正在校验…';
    return licenseJSON('/validate', memberIsLocalWorkbench() ? {} : { code: state.code, device_fingerprint: state.device_fingerprint || memberDeviceFingerprint() })
      .then(function (doc) { return persistMemberLicense(doc, state && state.code || ''); })
  ...
  function loadMemberLicense() {
    if (memberIsLocalWorkbench()) {
      return fetchJSON('/api/system/status', 'no-store').then(function (doc) {
        var local = doc.license || {}, active = !!local.valid;
        memberLicenseState = { valid: active, code: '', cloud_user_id: local.member_id || '',
          user: { id: local.member_id || '', plan: local.plan || '', expire_date: local.expire_date || '',
            remaining_days: local.remaining_days }, last_check: local.checked_at || '' };
        if (active && local.member_id) localStorage.setItem('jinshi_member_id', local.member_id);
        renderMemberLicense(memberLicenseState, active ? '已从本机安全授权缓存恢复' : '本机尚未授权或需要重新联网校验');
        if (!active && local.member_id) return validateMemberLicense();
        if (active && signalRealtimeMode) startSignalRealtime();
        if (active && stratRealtime) { window.clearTimeout(stratRealtimeTimer); refreshStratRealtime(); }
        return memberLicenseState;
      }).catch(function (err) { renderMemberLicense(null, '授权状态读取失败：' + err.message); return null; });
    }
    ...
    if (currentView === 'signal' || currentView === 'leading') {
      ...
      var signalView = signalRealtimeMode && signalRealtimePayload && signalRealtimePayload.available
        ? Object.assign({}, view, { date: signalRealtimePayload.data_date, data_date: signalRealtimePayload.data_date,
            ...
            leading_reason: signalDataIsToday ? (signalRealtimePayload.leading_reason || []) : (view.leading_reason || []),
  ...
  function refreshSignalRealtime() {
    ...
    fetchIntradayCore().then(function (r) {
      if ((currentView !== 'signal' && currentView !== 'leading' && currentView !== 'history') || !signalRealtimeMode) return;
  ...
  function refreshThemeRealtime() {
    if (!themeRealtime || currentView !== 'theme') return;
    fetchIntradayCore().then(function (result) {
  ...
  function buildStratRealtimeRows(payload, frozen) {
    ...
    return { rows: rows,
             ...
             status: (isMarketSession() ? '今日实时 · ' : '今日快照（非交易时段） · ') + (liveTime || '--:--:--') + ' · 命中 ' + rows.length + ' 只 · 行情时间如上（买点/止损定格 ' + ((frozen && frozen.date) || '-') + '）' };
  }
  ...
  function refreshStratRealtime() {
    ...
    Promise.all([fetchIntradayCore().catch(function () { return null; }),
      ...
      var memberToday = memberIsLocalWorkbench() && rs[2] && rs[2].available && rs[2].data_date === localToday();
      var live = payload && payload.available && payload.data_date === localToday() && (isMarketSession() || memberToday);
      if (!live) {
  ...
  if (memberIsLocalWorkbench()) loadMemberLicense();
  else if (memberLicenseState && memberLicenseState.code) validateMemberLicense();
