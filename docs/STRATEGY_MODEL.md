...
| ⑦ | `perfect_ten` | 十全十美 | money | 7项**全部满足**（仅 OHLCV，当前无 L2 数据）：MACD金叉＋MA5上行＋KDJ金叉＋RSI短＞长＋收盘站BBI＋量能确认＋形态过滤（不追高）；L2 条件（主力＞散户/LWR金叉/MMS＞MMM）待数据源补全后接入，届时 `min_conditions` 调回 11 | 满足条件数（最严苛，命中即核心候选） |
...
```json
{
  ...
  "models": {
    ...
    "perfect_ten": {
      ...
      "params": { "min_conditions": 7 },  # 7 条件仅 OHLCV；L2 数据补全后可调回 11
...
- ⑦十全十美：当前 7 项（仅 OHLCV，无 L2）→ `min_conditions` 默认 7；接入 L2（主力/MMS/MMM）后补足 11 项并调回 11
